...
class DataParser:
    ...
    def find_or_create_program(
        self, session, name: str, plan_year: int = None
    ) -> Program:
        ...
        program = (
            ...
            .filter(Program.name == name, Program.plan_year == plan_year)
            .first()
        )
    ...
    def find_or_create_subject(
        self, session, code: str, name: str, program: Program = None, **kwargs
    ) -> Subject:
        ...
        with get_db_session() as session:
            ...
            for subject_data in parsed_data:
                try:
                    ...
                    if subject_data["code"] not in [
                        s["code"]
                        for s in parsed_data[: parsed_data.index(subject_data)]
                    ]:
                        subjects_created += 1
                    ...
                    logger.error(
                        f"Error processing subject {subject_data.get('code', 'unknown')}: {e}"
                    )
                    continue

        logger.info(
            f"Stored {subjects_created} subjects and {offerings_created} offerings to database"
        )
    ...
    def store_previas_data(self, previas_data: Dict[str, Any]):
            ...
            for subject_code, prereq_info in previas_data.items():
                try:
                    ...
                    offering = (
                        session.query(Offering)
                        .filter(
                            Offering.subject_id == subject.id, Offering.type == "COURSE"
                        )
                        .first()
                    )
        ...
        logger.info(
            f"Stored {requirements_created} prerequisite requirements to database"
        )
