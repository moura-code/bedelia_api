...
from sqlalchemy import (
    ...
    Text, Numeric, SmallInteger, UUID, ForeignKey, CheckConstraint,
...
offering_type_enum = ENUM(
    'COURSE', 'EXAM',
    name='offering_type',
    create_type=False
)

group_scope_enum = ENUM(
    'ALL', 'ANY', 'NONE',
    name='group_scope', 
    create_type=False
)
...
class Subject(Base):
    ...
    offerings = relationship("Offering", back_populates="subject", cascade="all, delete-orphan")
...
class Offering(Base):
    ...
    links = relationship("OfferingLink", back_populates="offering", cascade="all, delete-orphan")
    requirement_groups = relationship("RequirementGroup", back_populates="offering", cascade="all, delete-orphan")
...
class OfferingLink(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    offering_id = Column(UUID(as_uuid=True), ForeignKey('offerings.id', ondelete='CASCADE'), nullable=False)
    kind = Column(Text, nullable=False)  # 'SYLLABUS', 'MOODLE', 'PROGRAM', 'GITHUB', etc.
...
class RequirementGroup(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("(scope <> 'ANY') OR (min_required IS NULL OR min_required >= 1)"),
        CheckConstraint("(scope <> 'ALL') OR (min_required IS NULL OR min_required >= 1)")
    )
    
    # Relationships
    offering = relationship("Offering", back_populates="requirement_groups")
    items = relationship("RequirementItem", back_populates="group", cascade="all, delete-orphan")
    parent_links = relationship("RequirementGroupLink", foreign_keys="RequirementGroupLink.child_group_id", back_populates="child_group")
    child_links = relationship("RequirementGroupLink", foreign_keys="RequirementGroupLink.parent_group_id", back_populates="parent_group")
...
class RequirementGroupLink(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    parent_group_id = Column(UUID(as_uuid=True), ForeignKey('requirement_groups.id', ondelete='CASCADE'), nullable=False)
    child_group_id = Column(UUID(as_uuid=True), ForeignKey('requirement_groups.id', ondelete='CASCADE'), nullable=False)
    order_index = Column(Integer, nullable=False, default=0)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    ...
    # Relationships
    parent_group = relationship("RequirementGroup", foreign_keys=[parent_group_id], back_populates="child_links")
    child_group = relationship("RequirementGroup", foreign_keys=[child_group_id], back_populates="parent_links")
...
class RequirementItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    group_id = Column(UUID(as_uuid=True), ForeignKey('requirement_groups.id', ondelete='CASCADE'), nullable=False)
    target_type = Column(target_type_enum, nullable=False)
    target_subject_id = Column(UUID(as_uuid=True), ForeignKey('subjects.id', ondelete='CASCADE'))
    target_offering_id = Column(UUID(as_uuid=True), ForeignKey('offerings.id', ondelete='CASCADE'))
...
class SubjectEquivalence(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    subject_id_a = Column(UUID(as_uuid=True), ForeignKey('subjects.id', ondelete='CASCADE'), nullable=False)
    subject_id_b = Column(UUID(as_uuid=True), ForeignKey('subjects.id', ondelete='CASCADE'), nullable=False)
...
class AuditSource(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    offering_id = Column(UUID(as_uuid=True), ForeignKey('offerings.id', ondelete='CASCADE'))
    url = Column(Text, nullable=False)
...
class DependencyEdge(Base):
    ...
    from_subject_id = Column(UUID(as_uuid=True), ForeignKey('subjects.id', ondelete='CASCADE'))
    from_offering_id = Column(UUID(as_uuid=True), ForeignKey('offerings.id', ondelete='CASCADE'))
    to_offering_id = Column(UUID(as_uuid=True), ForeignKey('offerings.id', ondelete='CASCADE'), nullable=False)
    group_id = Column(UUID(as_uuid=True), ForeignKey('requirement_groups.id', ondelete='CASCADE'), nullable=False)
...
Index('idx_req_groups_offering', RequirementGroup.offering_id, RequirementGroup.order_index)
Index('idx_req_group_links_parent', RequirementGroupLink.parent_group_id, RequirementGroupLink.order_index)
