...
from models import Base, Subject, Offering, Program
...
def import_json_data(json_file):
    ...
    try:
        ...
        if json_file.endswith('table_data.json') or json_file.endswith('table_data_backup.json'):
            # Import subjects data
            subjects_count, offerings_count = parser.store_scraped_data(
                data, 
                program_name="Imported Program"
            )
            logger.info(f"Imported {subjects_count} subjects and {offerings_count} offerings")
            
        elif json_file.endswith('previas_data.json') or json_file.endswith('previas_data_backup.json'):
...
def main():
    ...
    parser.add_argument('action', choices=[
        'setup', 'drop', 'test', 'stats', 'list', 'import'
